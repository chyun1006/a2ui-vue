...
export class A2UIManager {
  ...
  updateData(surfaceId, path, value) {
    ...
    console.log(`[updateData] surfaceId: ${surfaceId}, path: ${normalizedPath}, value:`, value)
    ...
    if (!normalizedPath || normalizedPath === '/') {
      ...
      // setValueByPath 直接修改 dataModel，不需要重新赋值
      setValueByPath(dataModel, normalizedPath, value)
    }

    console.log('[updateData] Updated dataModel:', this.state.dataModels.get(surfaceId))
  ...
  getData(surfaceId, path) {
    ...
    if (!path || path === '/') {
      console.log('[getData] Returning full dataModel:', dataModel)
      return dataModel
    }

    const value = getValueByPath(dataModel, path)
    console.log(
      `[getData] surfaceId: ${surfaceId}, path: ${path}, value:`,
      value,
      'dataModel:',
      dataModel,
    )
