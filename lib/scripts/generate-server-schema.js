...
const TEMPLATE_SCHEMA_PATH = path.resolve(__dirname, '../requirements/schemas/server_to_client_with_standard_catalog.json')
const MANIFEST_PATH = path.resolve(__dirname, '../src/a2ui-spec/a2ui-manifest.json')
const OUTPUT_PATH = path.resolve(__dirname, '../src/a2ui-spec/server_to_client_with_standard_catalog.json')
...
function generateServerSchema() {
  ...
  const componentPropertiesPath = templateSchema.properties.surfaceUpdate.properties.components.items.properties.component.properties

  console.log(`🔄 替换组件定义 (原有 ${Object.keys(componentPropertiesPath).length} 个组件) -> (新 ${Object.keys(manifest.components).length} 个组件)`)
